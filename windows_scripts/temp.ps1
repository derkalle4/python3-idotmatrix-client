# PowerShell script to update the video converter with file selection dialog
...
New-Item -ItemType Directory -Path "$root\utils\gif_converter" -Force | Out-Null

# Create the updated video_converter.py file
Write-Host "Creating updated video_converter.py file..."
...
class VideoToGifDialog(QDialog):
    ...
    def closeEvent(self, event):
        ...
        event.accept()
'@
Set-Content -Path "$root\utils\gif_converter\video_converter.py" -Value $videoConverterContent -Encoding UTF8

Write-Host "File updated successfully with directory selection dialog!"
Write-Host "The convert_video method now includes a dialog to select the output directory and filename."